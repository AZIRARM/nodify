...
public class ContentNodeHandler {
        ...
        public Flux<ContentNode> findAllByNodeCode(String code,
                        ...
                        boolean fillValues,
                        boolean withFiles) {
                return this.nodeHelper.evaluateNode(code, status).map(node -> code)
                                .map(nodeCode -> this.contentNodeRepository
                                                .findAllByNodeCodeAndStatus(nodeCode, status.name())
                                                .map(contentNode -> this.findContentNodeByCode(contentNode.getCode(),
                                                                status, translation, withFiles))
                                                .flatMap(Mono::from)
                                                ...
                                                .flatMap(contentNode -> this.contentHelper.fillContents(contentNode,
                                                                status, translation))
        ...
        private Mono<Node> evaluateNodeByCodeContent(String codeContent, StatusEnum status) {
                return this.contentNodeRepository.findByCodeAndStatus(codeContent, status.name())
                                .map(contentNode -> this.nodeHelper.evaluateNode(contentNode.getParentCode(), status))
                                .flatMap(Mono::from);
        }

        public Mono<ContentNode> findByCodeAndStatus(String code,
                        StatusEnum status,
                        String translation,
                        boolean withFiles) {
                return this.evaluateNodeByCodeContent(code, status)
                                .map(node -> this.findContentNodeByCode(code, status, translation, withFiles)
                                                .flatMap(this::addDisplay))
                                .flatMap(Mono::from);
        }

        public Mono<ContentNode> findBySlugAndStatus(String slug,
                        StatusEnum status,
                        String translation,
                        boolean withFiles) {
                return this.contentNodeRepository.findBySlugAndStatus(slug, status.name())
                                .map(contentNode -> contentNode.getCode())
                                .flatMap(code -> this.evaluateNodeByCodeContent(code, status)
                                                .map(node -> this.findContentNodeByCode(code, status, translation,
                                                                withFiles)
                                                                .flatMap(this::addDisplay)))
                                .flatMap(Mono::from);
        }
        ...
        private Mono<ContentNode> findContentNodeByCode(String code,
                ...
                return this.contentNodeRepository.findByCodeAndStatus(code, status.name())
                                .filter(contentNode -> withFiles || !contentNode.getType().equals(ContentTypeEnum.FILE)
                                                && !contentNode.getType().equals(ContentTypeEnum.PICTURE))
                                .flatMap(contentNode -> this.contentHelper
                                                .fillContents(contentNode, status, translation)
        ...
        private ContentNode addStatusParamIfNeeded(ContentNode contentNode) {
                ...
                Matcher matcher = urlPattern.matcher(ObjectUtils.isNotEmpty(html) ? html : "");
        ...
        public Mono<ContentFile> findResourceByCode(String code, StatusEnum status) {
                return this.evaluateNodeByCodeContent(code, status)
                                .map(node -> this.contentNodeRepository.findByCodeAndStatus(code, status.name())
                                                .map(contentNodeMapper::fromEntity)
                                                .flatMap(contentNode -> RulesUtils.evaluateContentNode(contentNode)
                                                                .filter(aBoolean -> aBoolean)
                                                                .map(aBoolean -> contentNode))
        ...
        public Mono<ContentFile> findResourceBySlug(String slug, StatusEnum status) {
                return this.contentNodeRepository.findBySlugAndStatus(slug, status.name())
                                .map(contentNode -> contentNode.getCode())
                                .flatMap(code -> this.evaluateNodeByCodeContent(code, status)
                                                .map(node -> this.contentNodeRepository
                                                                .findByCodeAndStatus(node.getCode(), status.name())
                                                                .map(contentNodeMapper::fromEntity)
                                                                .flatMap(contentNode -> RulesUtils
                                                                                .evaluateContentNode(contentNode)
                                                                                .filter(aBoolean -> aBoolean)
                                                                                .map(aBoolean -> contentNode))
                                                                .filter(contentNode -> ObjectUtils
                                                                                .isNotEmpty(contentNode.getFile()))
                                                                .flatMap(this::addDisplay)
                                                                .map(ContentNode::getFile)))
                                .flatMap(Mono::from);
        }
