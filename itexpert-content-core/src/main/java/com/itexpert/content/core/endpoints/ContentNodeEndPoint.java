...
public class ContentNodeEndPoint {
                ...
                .flatMap(hasRole -> {
                    ...
                    return SecurityUtils.hasRole(RoleEnum.EDITOR.name())
                            .flatMap(isEditor -> {
                                ...
                                return SecurityUtils.getUsername()
                                        .flatMap(user -> contentNodeHandler.publish(code, publish, user)
                                                .flatMap(contentNodeHandler::setPublicationStatus))
                                        .map(ResponseEntity::ok);
                            });
                });
                ...
                .flatMap(hasRole -> {
                    ...
                    return SecurityUtils.hasRole(RoleEnum.EDITOR.name())
                            .flatMap(isEditor -> {
                                ...
                                return SecurityUtils.getUsername()
                                        .flatMap(user -> contentNodeHandler.publishVersion(code, version, user))
                                        .map(ResponseEntity::ok);
                            });
                });
    }
    ...
    public Mono<ResponseEntity<ContentNode>> save(@RequestBody ContentNode contentNode) {
        return SecurityUtils.hasAnyRole(RoleEnum.ADMIN.name(), RoleEnum.EDITOR.name())
                .flatMap(hasRole -> {
                    ...
                    return SecurityUtils.hasRole(RoleEnum.EDITOR.name())
                            .flatMap(isEditor -> {
                                if (isEditor && !hasProjectAccess(contentNode.getCode())) {
                                    return Mono.just(new ResponseEntity<>(HttpStatus.FORBIDDEN));
                                }
                                return SecurityUtils.getUsername()
                                        .flatMap(user -> {
                                            contentNode.setModifiedBy(user);
                                            return Mono.defer(() -> contentNodeHandler.save(contentNode))
                                                    .flatMap(contentNodeHandler::setPublicationStatus)
                                                    .map(ResponseEntity::ok);
                                        });
                            });
    ...
    @GetMapping("/deleted")
    public Flux<ContentNode> getDeleted(@RequestParam(required = false, name = "parent") String parent) {
